...
import { MoflayDefaultError } from "../models/errors/moflaydefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | MoflayDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | MoflayDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new MoflayDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new MoflayDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
